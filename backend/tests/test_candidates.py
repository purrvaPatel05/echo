...
async def test_candidates_for_combines_distance_insurance_and_slots(session_factory):
    ...
    # internally. In the last hour before UTC midnight that is already "tomorrow", so the
    # expected day count is computed rather than assumed to be 0.
    ...
    assert chen.days_until_slot == (later_today.date() - datetime.now(UTC).date()).days
