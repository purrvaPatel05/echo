...
async def test_directory_and_seeded_referrals(api):
    ...
    refs = (await c.get("/referrals")).json()  # doc_001's own referrals only
    assert {r["id"] for r in refs} == {"ref_demo_knee", "ref_demo_urgent_chest"}
    assert all(r["status"] == "draft" for r in refs)


async def test_create_referral_uses_the_signed_in_physician(api):
    body = {"patient_id": "pat_001", "case_notes": "knee pain"}
    created = await api.client.post("/referrals", json=body)  # no physician id in the body
    assert created.status_code == 201 and created.json()["status"] == "draft"
    assert created.json()["referring_physician_id"] == "doc_001"
    assert (
        await api.client.post("/referrals", json={**body, "patient_id": "nope"})
    ).status_code == 404
    spoofed = await api.client.post(
        "/referrals", json={**body, "referring_physician_id": "doc_002"}
    )
...
async def test_patient_summary_wording_reads_naturally(api):
    await _ready_to_match(api)
    run = await _match(api)
    await _approve(api, KNEE, run, KNEE_DOC)
    summary = (await api.client.get(f"/referrals/{KNEE}/patient-summary")).json()["summary"]
    assert "a specialist in Orthopedics at Charles River Orthopedics" in summary
