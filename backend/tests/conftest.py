...
class Api:
    ...
    def as_physician(self, physician_id: str) -> AsyncClient:
        """Act as this physician (dev-mode header) for the following requests."""
        self.client.headers["X-Dev-Physician"] = physician_id
        return self.client


@pytest.fixture(autouse=True)
def dev_auth(monkeypatch):
    """Most tests exercise behavior, not login: run in dev mode (auth is tested in test_auth)."""
    monkeypatch.setattr(settings, "auth_mode", "dev")


@pytest.fixture(autouse=True)
def no_simulated_colleague(monkeypatch):
    """Tests must not depend on a developer's .env: the demo colleague is off unless enabled."""
    monkeypatch.setattr(settings, "simulate_colleague_replies", False)
...
async def api():
    ...
    async with AsyncClient(
        ...
        base_url="http://test",
        headers={"X-Dev-Physician": "doc_001"},
    ) as client:
